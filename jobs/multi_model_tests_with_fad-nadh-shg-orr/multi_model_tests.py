...
def main():
    ...
    train_data = NSCLCDataset('NSCLC_Data_for_ML', ['fad', 'nadh', 'shg', 'orr'], device=torch.device('cpu'),
    ...
    eval_test_data = NSCLCDataset('NSCLC_Data_for_ML', ['fad', 'nadh', 'shg', 'orr'], device=torch.device('cpu'),
                                  label='Metastases', mask_on=True)
    ...
    print(f'Training patients: {train_pts}.\nEvaluation patients: {eval_pts}.\nTest patients: {test_pts}.\n'
          f'CHECK -- Combined Eval-Test patients: {comb_pts}. '
          f'Non-metastatic: {comb_image_counts[1]}. Metastatic: {comb_image_counts[0]}\n')
    ...
    for i, model in enumerate(models):
        ...
        for ep in epochs:
            ...
            scores, fig = score_model(model, test_loader, print_results=True, make_plot=True,
                                      threshold_type='roc')
            fig.savefig(f'outputs/{model.name}/plots/best_eval_{model.name}_on_test_plots.png')
            plt.close(fig)
            with open(f'outputs/{model.name}/results.txt', 'a') as f:
            ...
            scores, fig = score_model(model, comb_loader, print_results=True, make_plot=True,
                                      threshold_type='roc')
            fig.savefig(f'outputs/{model.name}/plots/best_eval_{model.name}_on_eval-test_plots.png')
            plt.close(fig)
            with open(f'outputs/{model.name}/results.txt', 'a') as f:
